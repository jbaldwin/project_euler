...
ciper1.txt (right click and 'Save Link/Target As...'), a file containing the encrypted ASCII codes,
...
function decrypt($encrypted) {
	...
	for($c = 97; $c <= 122; $c++) {
		...
		$plaintext = "";
		for($i = 0; $i < count($encrypted); $i++) {
			$plaintext .= chr(($encrypted[$i] ^ $key[$i % count($key)]));
		}
...
$encrypted = file_get_contents("cipher1.txt");
