...
The product of these numbers is 26 x 63 x 78 x14 = 1788696.

What is the greatest product of four adjacent numbers in the same direction (up, down, left, right,
or diagonally) in the 20 x 20 grid?
**/

$grid = array(
	array(8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 05, 7, 78, 52, 12, 50, 77, 91, 8),
	...
	array(86, 56, 0, 48, 35, 71, 89, 7, 05, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58),
	array(19, 80, 81, 68, 5, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 4, 89, 55, 40),
	array(4, 52, 08, 83, 97, 35, 99, 16, 7, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66),
...
for($i = 0; $i < 20; $i++) {
	for($j = 0; $j < 20; $j++) {
		...
		// left
		if($i > 3) {
			$max = max($max, $grid[$i][$j] * $grid[$i-1][$j] * $grid[$i-2][$j] * $grid[$i-3][$j]);
		}
		// up left
		if($i > 3 && $j > 3) {
			$max = max($max, $grid[$i][$j] * $grid[$i-1][$j-1] * $grid[$i-2][$j-2] * $grid[$i-3][$j-3]);
		}
		// down left
		if($i > 3 && $j < 17) {
			$max = max($max, $grid[$i][$j] * $grid[$i-1][$j+1] * $grid[$i-2][$j+2] * $grid[$i-3][$j+3]);
		...
		// up
		if($j > 3) {
			$max = max($max, $grid[$i][$j] * $grid[$i][$j-1] * $grid[$i][$j-2] * $grid[$i][$j-3]);
		}
		// down
		if($j < 17) {
			$max = max($max, $grid[$i][$j] * $grid[$i][$j+1] * $grid[$i][$j+2] * $grid[$i][$j+3]);
